...
def main():
    ...
    if not audit.get("audit_passed") or audit.get("complete_pairs") != 225:
        raise RuntimeError("Raw download audit gate is not satisfied")
    ...
    if output_manifest.exists():
        raise FileExistsError("Versioned replay manifest already exists")
    output_manifest.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))
