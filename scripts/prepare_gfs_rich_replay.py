...
from jalrakshak_ml.gfs_replay.grib import source_uri
...
def main() -> int:
    ...
    log.info(
        "Total Events: %d (Train: %d, Validation: %d, Test: 0)",
        len(events),
        len(train_ids),
        len(val_ids),
    )
    ...
    log.info(
        "Total Issues to Generate: %d across %d events", plan["total_issues"], plan["total_events"]
    )
    ...
    if args.dry_run_download:
        ...
        log.info(
            "Total Unique Granules: %d (No full ~450MB GRIB download; byte ranges only)",
            plan["num_unique_cycle_leads"],
        )
        # Sample first and last granule
        first_gl = plan["unique_cycle_leads_required"][0]
        last_gl = plan["unique_cycle_leads_required"][-1]
        log.info(
            "Sample Granule [0]: %s f%03d -> %s",
            first_gl["cycle"],
            first_gl["lead"],
            source_uri(first_gl["cycle"], first_gl["lead"]),
        )
        log.info(
            "Sample Granule [-1]: %s f%03d -> %s",
            last_gl["cycle"],
            last_gl["lead"],
            source_uri(last_gl["cycle"], last_gl["lead"]),
        )
        log.info(
            "DRY-RUN COMPLETE: Zero bytes retrieved over network. URLs, selectors, and destinations proven."
        )
        return 0
    ...
    if not args.download:
        log.info(
            "Live download flag (--download) not supplied. Stopping before network operations."
        )
        log.info("To perform real download when ready, run with --download.")
        return 0

    raw_cache = resolved_output / "raw_gfs_cache"
    top_manifest = initialize_download_manifest(plan, raw_cache)
    top_manifest_path = resolved_output / "download_manifest.json"
    top_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    top_manifest_path.write_text(json.dumps(top_manifest, indent=2), encoding="utf-8")
    completed = incomplete = failed = measured = 0
    pair_records = []
    for pair in plan["unique_cycle_leads_required"]:
        try:
            result = acquire_rich_granule(pair["cycle"], pair["lead"], raw_cache)
            state = result["completion_state"]
            completed += state == "COMPLETE"
            incomplete += state != "COMPLETE"
            failed += bool(result.get("failed_variables"))
            measured += int(result["total_measured_bytes"])
            ...
            top_manifest.update(
                {
                    ...
                    "missing_pairs": 225 - completed - incomplete,
                    "failed_pairs": failed,
                    "total_measured_bytes": measured,
                    "pair_states": pair_records,
                    "execution_state": "COMPLETE" if completed == 225 else "IN_PROGRESS",
                }
            )
            part = top_manifest_path.with_suffix(".json.part")
            part.write_text(json.dumps(top_manifest, indent=2), encoding="utf-8")
            part.replace(top_manifest_path)
    return 0 if completed == 225 else 1
